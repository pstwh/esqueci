...
template_module = Blueprint("template", __name__, url_prefix="/template")
...
@template_module.route("/template")
def index_template():
    files = file.find({})
    return files


@template_module.route("/template/create", methods=["POST"])
def create_template():
...
@template_module.route("/template/<string:template_name>")
def retrieve_template(template_name):
    try:
        ...
        return send_file(
            ...
            as_attachment=True,
            attachment_filename=f"{template_name}.zip",
        )
    except:
